...
// PATCH /api/air-emissions/[id] — edit a record. Approved/rejected records are
// locked (CLAUDE.md). Editing a RETURNED record re-enters the workflow as SUBMITTED.
export async function PATCH(req: NextRequest, { params }: Context) {
  ...
  const record = await db.airEmissionRecord.update({
    ...
    data: {
      ...
      // A returned record goes back into the queue on resubmit.
