...
import { gasFormSchema, type GasFormValues } from "@/lib/validations/gas";
import {
  Form,
  FormControl,
  FormField,
...
export function GasForm({
  ...
  recordId?: string;
  defaultValues?: Partial<GasFormValues>;
  onSuccess: () => void;
}) {
  ...
  function submit(values: GasFormValues) {
    startTransition(async () => {
      try {
        const url = recordId ? `/api/gas/${recordId}` : "/api/gas";
        ...
        if (res.ok) {
          toast.success(recordId ? "Record updated" : "Record saved");
          router.refresh();
          onSuccess();
          return;
        }
        ...
        if (data.fieldErrors) {
          for (const [field, message] of Object.entries(data.fieldErrors)) {
            if (field in EMPTY) {
              form.setError(field as keyof GasFormValues, { message });
            }
          }
        }
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input inputMode="decimal" placeholder="12400" {...field} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
